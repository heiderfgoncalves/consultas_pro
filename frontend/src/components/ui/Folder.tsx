...
const Folder: React.FC<FolderProps> = ({ color = '#3b82f6', size = 1, items = [], className = '' }) => {
  ...
  const handleClick = (e: React.MouseEvent) => {
    // Evita conflito com cliques nos cartões internos se o usuário clicar neles
    const target = e.target as HTMLElement;
    if (target.closest('.paper') && open) {
      return;
    }
    setOpen(prev => !prev);
    if (open) {
  ...
  const scaleStyle = { transform: `scale(${size})`, display: 'inline-block' } as React.CSSProperties;
      ...
      <div className={folderClassName} style={folderStyle} onClick={handleClick}>
        <div className="folder__back">
          {papers.map((item, i) => (
            ...
            >
              <div className="paper-content h-full w-full relative overflow-hidden rounded-md">
                {item}
              </div>
            </div>
          ))}
