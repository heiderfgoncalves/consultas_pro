...
export function RealtimeConsultationsTab({ accessToken }: { accessToken: string | null }) {
  ...
  const downloadPdf = useCallback(async (item: any) => {
    if (pdfLoadingIds[item.id]) return;
    setPdfLoadingIds(prev => ({ ...prev, [item.id]: true }));
    try {
      const detail = await getAdminConsultationDetail(accessToken, item.id);
      
      const realData = detail.renderPayload || detail.mergedPayload || {};
      const templateLayout = detail.template?.layout ?? null;
      ...
      if (!pagesHtml) {
        pagesHtml = `<section class="page" style="width:794px;min-height:1123px;padding:48px;background:#fff">
          <h1 style="font-size:22px;font-weight:800;margin:0 0 8px">${detail.template?.name || 'Relatório'}</h1>
          <p style="font-size:13px;color:#64748b;margin:0 0 24px">Documento: ${detail.subjectDocument}</p>
...
<script>if(typeof lucide!=='undefined'){lucide.createIcons();}</script>
      ...
      frame.onload = async () => {
        try {
          ...
          const docName = detail.template?.name || 'Relatorio';
          const safeDoc = detail.subjectDocument?.replace(/\D/g, '') || 'sem-doc';
          ...
          frame.remove();
          setPdfLoadingIds(prev => ({ ...prev, [item.id]: false }));
        }
      };
    } catch (err: any) {
      console.error('Erro ao preparar PDF:', err);
      toast.error('Erro ao buscar dados completos da consulta para o PDF');
      setPdfLoadingIds(prev => ({ ...prev, [item.id]: false }));
    }
  }, [accessToken, pdfLoadingIds]);
