...
const stepsData = [
  ...
  { icon: Shuffle, label: "DE-PARA", desc: "Motor flat normaliza payloads heterogêneos em variables únicas." },
    ...
    <section id="pipeline" className="relative py-6 border-t border-hairline">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mt-10 relative">
          ...
          <div className="absolute left-0 right-0 top-[26px] h-px bg-hairline hidden md:block" style={{ zIndex: 0 }} />
          
          {/* Linha com fluxo ativo de elétrons em SVG */}
          <div className="absolute left-0 right-0 top-[24px] h-1.5 hidden md:block overflow-hidden pointer-events-none" style={{ zIndex: 0 }}>
            <svg className="w-full h-full">
              <line 
                ...
                x2="100%" 
                y2="2" 
                className="stroke-brand/50 stroke-[1.5] animate-pipeline-flow" 
                style={{ 
          ...
          <LayoutGroup id="seven-steps-layout-group">
            <div className="grid grid-cols-2 md:grid-cols-7 gap-x-4 gap-y-10 relative z-10">
                  ...
                  <FadeUp key={i} delay={0.1 + i * 0.1}>
                    ...
                    >
                      ...
                      <div className="absolute inset-0 border border-dashed border-brand/20 bg-brand/[0.02] rounded-md flex flex-col items-center justify-center pointer-events-none z-0 select-none transition-all duration-300 group-hover:border-brand/35">
                        <span className="mono text-[9px] tracking-[0.2em] text-brand/50 uppercase">Etapa 0{i + 1}</span>
                        <span className="mono text-[7px] tracking-widest text-muted-foreground/60 mt-1">[ BERÇO_ÂNCORA ]</span>
                      </div>
                      ...
                      <InteractiveModularCard 
                        ...
                        index={i}
                        onSwap={handleSwap}
                        className="p-4 w-full h-full flex flex-col items-center text-center bg-background/50 hover:bg-background/80 transition-colors duration-300 z-10"
                      >
