...
export function InteractiveModularCard({
  ...
  // Executa a verificação física de colisão ao soltar
  const handleDragEnd = (event: any, info: any) => {
    setIsDragging(false);
    
    if (dragGroup !== undefined && index !== undefined && onSwap && cardRef.current) {
      // Coletar todos os berços (slots) do mesmo grupo
      const slots = document.querySelectorAll(`.grid-anchor-slot[data-grid-group="${dragGroup}"]`);
      ...
      if (slots.length > 0) {
        ...
        // Tolerância de encaixe elástico generosa
        const maxAcceptableDistance = Math.max(cardRect.width * 1.5, 300);
        
        if (closestSlot && minDistance < maxAcceptableDistance) {
          const targetIdx = parseInt(closestSlot.getAttribute("data-slot-index") || "0", 10);
          
          if (targetIdx !== index) {
            // Disparar o callback para reordenar os dados no componente pai
            onSwap(index, targetIdx, dragGroup);
            // Reseta os offsets locais para (0, 0) para que o card use a posição natural do novo slot
            x.set(0);
            y.set(0);
            return;
          }
    ...
    // Snap elástico (mola) de volta ao berço de origem se não encaixou em outro slot válido
  ...
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 rounded-md pointer-events-none z-0 transition-opacity duration-300 ${
        isActive ? "bg-brand/5 animate-pulse opacity-100" : "bg-brand/[0.01] opacity-30"
      }`} />
      <div className={`absolute -inset-px rounded-md border pointer-events-none z-0 transition-opacity duration-300 ${
        isActive ? "border-brand/35 animate-pulse opacity-100" : "border-brand/10 opacity-10 group-hover/card:opacity-50"
      }`} />
      ...
      <div
        ...
        className={`absolute -top-3.5 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-surface border border-hairline text-muted-foreground transition-all duration-300 z-30 ${
          useDragHandle && (isHovered || isDragging) ? "cursor-grab active:cursor-grabbing pointer-events-auto" : "pointer-events-none"
      ...
      {!useDragHandle && (
        ...
        >
          ...
          <span className="mono text-[7px] tracking-wider uppercase">Dbl Clique p/ Reset</span>
        </div>
      )}
