...
import { useEditorStore, useActiveQuery, useActiveTemplate } from "../store";
  ...
  return (
    <Section title="Variáveis Dinâmicas">
      <div className="space-y-2">
        <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Sistêmicas (Clique para copiar)</div>
        {["$template.protocol", "$template.date", "$template.company"].map((p) => (
          <code
            key={p}
            onClick={() => copyVar(`{${p}}`)}
            className="block cursor-pointer rounded border border-border bg-chrome px-2 py-1 font-mono text-[11px] text-muted-foreground hover:border-accent hover:bg-accent/5 transition-all"
          >
            {`{${p}}`}
          </code>
        ))}
        <div className="pt-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Tipos e Campos (Clique para copiar)</div>
        {paths.slice(0, 40).map((p) => (
          <code
            key={p}
            onClick={() => copyVar(`{${p}}`)}
            className="block cursor-pointer truncate rounded border border-border bg-chrome px-2 py-1 font-mono text-[11px] text-muted-foreground hover:border-accent hover:bg-accent/5 transition-all"
            title={p}
          >
            {`{${p}}`}
          </code>
