...
import { documentToLegacy } from "../model/adapter";

import { type ProviderConsultation } from "@/types/integrations";

export type DropPos = { overId: string; side: "before" | "after" } | null;

export function EditorScreen({ consultations = [] }: { consultations?: ProviderConsultation[] }) {
  ...
  const queryTypes = useEditorStore((s) => s.queryTypes);
  ...
  const setQueryTypes = useEditorStore((s) => s.setQueryTypes);
  ...
  useEffect(() => {
    if (apiTemplates && apiTemplates.length > 0) {
      const mapped = apiTemplates.map(apiT => {
        if (apiT.layout && typeof apiT.layout === "object" && ("schemaVersion" in apiT.layout || "nodes" in apiT.layout)) {
          return documentToLegacy({
            id: apiT.id,
            name: apiT.name,
            ...(apiT.layout as any)
          }, apiT.id);
        } else if (apiT.layout && typeof apiT.layout === "object" && "sections" in apiT.layout) {
          const lObj = apiT.layout as any;
          return {
            id: apiT.id,
            name: apiT.name,
            sections: lObj.sections || [],
            selectedQueryBlocks: lObj.selectedBlockIds || [],
            logo: apiT.logo || undefined
          };
        } else {
          return {
            id: apiT.id,
    ...
    if (consultations.length > 0) {
      setQueryTypes(
        consultations.map((c) => ({
          id: c.id,
          name: c.name,
          description: c.description || "Consulta importada",
          price: c.consultationPrice || 0,
          icon: "FileSearch",
          category: "analise",
          sample: c.sampleResponse ? JSON.parse(c.sampleResponse) : {},
        }))
      );
    }
  }, [consultations, setQueryTypes]);
        ...
        <div className="flex h-full w-full flex-col bg-chrome">
          ...
          <div className="flex min-h-0 flex-1">
            <ResizablePanelGroup direction="horizontal">
              ...
              <ResizablePanel defaultSize={leftOpen && rightOpen ? 58 : leftOpen ? 78 : rightOpen ? 78 : 100} minSize={30} className="min-w-0">
                <CanvasColumn leftOpen={leftOpen} setLeftOpen={setLeftOpen} rightOpen={rightOpen} setRightOpen={setRightOpen} />
              </ResizablePanel>
              {rightOpen && (
                <>
                  <ResizableHandle withHandle />
