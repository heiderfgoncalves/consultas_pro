...
import { useSaveTemplate } from "../api/hooks";
import { legacyToDocument } from "../model/adapter";
...
export function EditorHeader() {
  ...
  const saveLocal = useEditorStore((s) => s.save);
  const dirty = useEditorStore((s) => s.dirty);
  const queryTypes = useEditorStore((s) => s.queryTypes);

  const { mutateAsync: saveToApi, isPending: isSaving } = useSaveTemplate();
  ...
  return (
    <div className="flex h-14 shrink-0 items-center gap-3 border-b border-border bg-background px-4">
        ...
        onClick={async () => { 
          ...
          try {
            await saveToApi({ 
              id: template.id, 
              name: template.name, 
              document: legacyToDocument(template),
              logo: template.logo 
            });
      ...
      >
        ...
        {isSaving ? "Salvando..." : `Salvar${dirty ? " *" : ""}`}
      </Button>
    </div>
  );
}
