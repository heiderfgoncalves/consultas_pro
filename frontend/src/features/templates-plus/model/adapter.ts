...
function fieldFromCanonical(n: TemplateFieldNode): FieldNode {
  ...
  return {
    ...
    children: n.children ? n.children.map((c) => fieldFromCanonical(c as TemplateFieldNode)) : undefined,
  };
}
...
function fieldToCanonical(f: FieldNode): TemplateFieldNode {
  return {
    ...
    children: f.children ? f.children.map(fieldToCanonical) : undefined,
    style: {
      fontSize: f.fontSize,
