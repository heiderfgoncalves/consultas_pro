...
export function SectionCard({ section, index, mode, context }: Props) {
  ...
  return (
    <div className="relative">
      ...
      <div className={["absolute -top-3 left-3 flex items-center gap-1 transition-opacity", hover || isSelected ? "opacity-100" : "opacity-0"].join(" ")}>
      ...
      <div className={["absolute -top-3 right-3 flex items-center gap-1 transition-opacity", hover || isSelected ? "opacity-100" : "opacity-0"].join(" ")}>
        {experience === "admin" && (
          <button
        ...
        <Popover>
          ...
          <PopoverContent align="end" className="w-44 p-1">
            <button onClick={() => duplicate(section.id)} className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-xs hover:bg-muted">
              <Copy className="h-3 w-3" /> Duplicar
            </button>
            <button onClick={() => remove(section.id)} className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-xs text-destructive hover:bg-destructive/10">
