...
export type SwitchProps = React.ComponentPropsWithoutRef<typeof SwitchPrimitives.Root> & {
  /** Compact track for dense toolbars and card headers */
  size?: "default" | "sm";
};
  ...
  ({ className, size = "default", ...props }, ref) => (
    <SwitchPrimitives.Root
      className={cn(
        ...
        size === "sm" && "h-4 w-7 border-border/50 data-[state=unchecked]:bg-muted/70",
        className,
      )}
      {...props}
      ref={ref}
    >
      <SwitchPrimitives.Thumb
        className={cn(
          ...
          size === "default" &&
            "h-5 w-5 shadow-lg data-[state=checked]:translate-x-5 data-[state=unchecked]:translate-x-0",
          size === "sm" && "h-3 w-3 shadow-sm data-[state=checked]:translate-x-3 data-[state=unchecked]:translate-x-0",
        )}
      />
    </SwitchPrimitives.Root>
  ),
