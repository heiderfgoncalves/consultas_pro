...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  const constants = useMemo(
    ...
    const tickerFn = () => {
      ...
      // Efeito magnético adaptativo: menor e mais preciso no início, mais tolerante em cards do meio/fim.
      if (isActiveRef.current && activeTargetRef.current) {
        ...
        const baseForce = area > 120000 ? 0.22 : area > 42000 ? 0.3 : 0.42;
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      while (current && current !== document.body && current !== document.documentElement) {
        if (current.matches) {
          ...
          const isExplicitTarget = current.matches(targetSelector);
          const isInteractiveShell =
            current.classList.contains('interactive-modular-card') &&
            current.dataset.cursorTarget !== 'true';
          const isVisibleTarget = rect.width >= 12 && rect.height >= 12;

          if ((isPointer || isExplicitTarget) && !isInteractiveShell && isVisibleTarget) {
