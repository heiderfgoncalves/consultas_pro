...
const defaultStages = [
  {
    ...
    n: "01",
    title: "PROVEDORES",
    desc: "Conecta APIs externas (SOLLOS, Serasa, Boa Vista, +37)",
    active: false,
  },
  {
    id: "stage-2",
    n: "02",
    title: "CONSULTAS",
    desc: "Define produtos consumíveis com preço por bloco",
    active: false,
  },
  {
    id: "stage-3",
    n: "03",
    title: "TIPOS",
    desc: "Motor de-para FLAT — normaliza payloads heterogêneos",
    active: true,
  },
  {
    id: "stage-4",
    n: "04",
    title: "TEMPLATES",
    desc: "Drawer drag-and-drop com motor math() BR",
    active: false,
  },
  {
    id: "stage-5",
    n: "05",
    title: "EMISSÃO",
    desc: "Fila BullMQ + ledger transacional com estorno",
    active: false,
  },
];
            ...
            >
              <HUDFrame className="p-6 md:p-8 h-full bg-transparent border-none">
                ...
                <p className="mt-3 text-sm text-muted-foreground max-w-md">
                  Da consulta crua ao relatório customizado. Arraste e solte as
                  etapas para simular como a operação organiza provedores,
                  templates, filas, ledger e entrega white-label.
                </p>
                ...
                <div className="mt-8 relative">
                  ...
                  <Reorder.Group
                    ...
                    className="workflow-flow-list space-y-3 md:space-y-4"
                  >
                    {orderedStages.map((s, idx) => {
                      ...
                      return (
                        <Reorder.Item
                          key={s.id}
                          value={s}
                          className={`workflow-flow-card ${isActive ? "is-active" : ""} group/step relative flex items-center gap-4 md:gap-5 pl-1 py-3 pr-3 md:py-3.5 md:pr-4 rounded-lg cursor-grab active:cursor-grabbing transition-all duration-300 z-10 ${
                            isActive
                        ...
                        >
                          ...
                          >
                            {isActive ? (
                              ...
                              <span className="text-xs font-mono relative z-10 font-bold">
                                {s.n}
                              </span>
                            )}
                          </div>

                          <div className="flex-1 flex flex-col transition-transform duration-300 ease-out group-hover/step:translate-x-2">
                            <span
                              className={`mono text-[11px] md:text-[12px] tracking-[0.15em] uppercase font-semibold ${isActive ? "text-brand font-bold" : "text-stone-300 group-hover/step:text-white"}`}
                            >
                              {s.title}
                            </span>
                ...
                <div className="mt-6 flex items-center justify-between mono text-[10px] tracking-[0.18em] uppercase">
                  <span className="text-muted-foreground">
                    MODO: <span className="text-foreground">PROD</span>
                  </span>
                  <span className="rounded-sm border border-brand/40 px-2 py-0.5 text-brand bg-brand/5">
                    AUDITÁVEL
                  </span>
                </div>
              </HUDFrame>
                ...
                >
                  <HUDFrame className="p-6 md:p-8 bg-transparent border-none">
                    ...
                    <div className="mt-4 flex items-end gap-4">
                      <span className="text-5xl md:text-6xl font-semibold tracking-[-0.03em] text-foreground">
                        40+
                      </span>
                      <span className="mb-2 text-sm text-muted-foreground">
                        provedores e produtos organizados em catálogo
                        administrável
                      </span>
                    </div>
                    ...
                    <div className="mt-6 grid grid-cols-4 gap-2">
                      {providers.map((p) => (
                        <div
                          key={p}
                          className="flex h-11 items-center justify-center rounded-lg border border-hairline bg-surface/30 mono text-[10px] tracking-[0.12em] uppercase text-foreground/80 hover:border-brand/40 hover:text-brand transition-colors cursor-target" data-cursor-target="true"
                        >
                          {p}
                        </div>
                      ))}
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="mono text-[12px] text-brand font-semibold mt-1">
        {value}
      </div>
    </div>
  );
